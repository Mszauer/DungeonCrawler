...
namespace Components {
    class ButtonComponent : Component{
        ...
        public override void OnUpdate(float dTime) {
            ...
            //is x inside?
            if (gameObject.GlobalPosition.X < i.MousePosition.X && i.MousePosition.X < (gameObject.GlobalPosition.X+buttonRenderer.SourceRects[buttonRenderer.CurrentSprite].Width)) {
                //y is inside?
                if (gameObject.GlobalPosition.Y < i.MousePosition.Y && i.MousePosition.Y < (gameObject.GlobalPosition.Y + buttonRenderer.SourceRects[buttonRenderer.CurrentSprite].Height)) {
                    if (DoHover != null) {
                        DoHover();
                    }
                }//end y
